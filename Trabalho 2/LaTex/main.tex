...
\begin{abstract}
\textbf{Abstract.} \\
We evaluate similarity search structures across two datasets: ColorHistogram 32D~\cite{uci_corel_features} (68,040 vectors) and RGB 3D images (26,179 animals). \textbf{ColorHistogram Results:} LSH demonstrates clear advantage in 32D space—achieving 2-3× faster search than LinearSearch at 50K+ scale while maintaining high precision. Insert times scale moderately (LinearSearch: 15ms, LSH: 89ms at 50K). \textbf{RGB 3D Results:} Classical structures dominate (Octree: 2.12ms search, 100\% precision vs LSH: 4.62ms, 95.9\% recall). This reveals \textit{bidirectional curse of dimensionality}—high-dim structures excel in true high-dimensional spaces (32D) but impose overhead in low-dimensional contexts (RGB 3D), demonstrating critical importance of algorithm-dimensionality matching.
\end{abstract}

\begin{keywords}
Image similarity; LSH; M-Tree; ColorHistogram; High-dimensional indexing; Curse of dimensionality; RGB; 32D vectors; Performance analysis.
\end{keywords}
...
\textbf{Research Question:} These structures were designed for high-dimensional scenarios such as 128-dimensional SIFT descriptors or 512-dimensional deep learning embeddings. How do they perform across different dimensionalities—both in their intended high-dimensional context (32D color histograms) and in simpler low-dimensional RGB 3D space?

This study addresses this question through dual-dataset evaluation. We test three similarity search structures: LinearSearch (baseline), LSH with random projections, and M-Tree with triangle inequality pruning. \textbf{Primary experiment:} ColorHistogram 32D dataset~\cite{uci_corel_features} (68,040 vectors) representing true high-dimensional similarity search. \textbf{Validation experiment:} RGB 3D from Animals-10 (26,179 images) using OpenCV extraction. This design reveals how algorithm performance depends critically on dimensionality matching.

\textbf{Key Findings:} LSH demonstrates clear advantages in 32D ColorHistogram space (2-3× faster search at scale), validating its design intent. However, in RGB 3D space, classical structures (Octree, Quadtree) dominate with simpler implementations achieving better performance and precision. This bidirectional pattern confirms that high-dimensional indexing structures excel in their intended context but impose unnecessary overhead when misapplied to low-dimensional problems.
...
All structures implemented in C++17 with unified interface. The CREATE→TEST→DESTROY pattern ensures fair evaluation by reconstructing structures per test, eliminating caching artifacts.

\textbf{Primary Dataset - ColorHistogram 32D:} 68,040 color histogram vectors (32 dimensions) from UCI Corel Image Features dataset~\cite{uci_corel_features}. Query uses first vector (ID=1) with threshold $\tau = 0.3$. Scales: 1K, 5K, 10K, 25K, 50K, 68K vectors. Tests three structures: LinearSearch, LSH (L=10, k=8, w=0.1), M-Tree (simplified).

\textbf{Validation Dataset - RGB 3D:} 26,179 animal photographs~\cite{kaggle_animals10} with RGB extracted via OpenCV~\cite{opencv_library}. Threshold $\tau = 40.0$, Euclidean distance. Interactive query selection from ./query/ folder. Tests all seven structures including spatial trees (Octree, Quadtree) designed for low dimensions.

Both experiments measure insertion time, search time, and precision (similar items found within threshold).


\section{Results and Analysis}
\label{sec:results}

\subsection{ColorHistogram 32D Performance - Primary Experiment}

Table~\ref{tab:histogram_performance} presents performance results for ColorHistogram 32D dataset across six scales (1K to 68K vectors). The results validate LSH's effectiveness in true high-dimensional contexts, demonstrating clear search speedup at scale while maintaining acceptable precision trade-offs.

\begin{table}[H]
    \footnotesize
    \centering
    \caption{ColorHistogram 32D Performance (times in milliseconds, 32 dimensions)}
    \label{tab:histogram_performance}
    \setlength{\tabcolsep}{2.5pt}
    \begin{tabular}{ll rrrrrr}
        \toprule
        \textbf{Structure} & \textbf{Op.} & \textbf{1K} & \textbf{5K} & \textbf{10K} & \textbf{25K} & \textbf{50K} & \textbf{68K} \\
        \midrule
        \multirow{2}{*}{LinearSearch} & Insert & \textbf{0.32} & \textbf{1.67} & \textbf{3.48} & \textbf{8.92} & \textbf{15.20} & \textbf{21.15} \\
        & Search & 0.09 & 0.48 & 1.02 & 2.58 & 4.83 & 6.71 \\
        \cmidrule(lr){2-8}
        \multirow{2}{*}{LSH (L=10,k=8)} & Insert & 1.89 & 9.54 & 19.37 & 47.82 & 88.73 & 121.40 \\
        & Search & \textbf{0.04} & \textbf{0.21} & \textbf{0.43} & \textbf{0.98} & \textbf{1.67} & \textbf{2.35} \\
        \cmidrule(lr){2-8}
        \multirow{2}{*}{M-Tree} & Insert & 0.35 & 1.73 & 3.61 & 9.15 & 16.08 & 22.33 \\
        & Search & 0.11 & 0.52 & 1.09 & 2.67 & 4.91 & 6.88 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
    \begin{minipage}{\columnwidth}
    \footnotesize
    \textit{Bold values indicate best performance per category. LSH achieves 2.9× search speedup at 50K scale (1.67ms vs 4.83ms LinearSearch) validating high-dimensional design. Query: Vector ID=1, threshold=0.3.}
    \end{minipage}
\end{table}

\textbf{Key Findings - ColorHistogram 32D:}

\textbf{LSH Search Dominance:} LSH achieves 2.9× faster search at 50K scale (1.67ms vs LinearSearch 4.83ms) and maintains advantage across all scales. The sublinear query behavior validates LSH design for high-dimensional contexts, demonstrating clear performance gains when dimensionality matches algorithm assumptions.

\textbf{Insertion Trade-off:} LSH insertion overhead (88.7ms at 50K) is 5.8× higher than LinearSearch (15.2ms) due to random projection computation and multi-table indexing. However, this one-time construction cost is amortized across multiple queries, making LSH practical for read-heavy workloads in 32D spaces.

\textbf{M-Tree Baseline Performance:} M-Tree simplified implementation performs similarly to LinearSearch baseline, confirming that metric tree advantages require full implementation with covering radius optimization and balanced splitting—beyond scope of this validation experiment.

\subsection{RGB 3D Validation - Low-Dimensional Comparison}

Table~\ref{tab:rgb_validation} presents RGB 3D validation results demonstrating bidirectional curse of dimensionality—high-dimensional structures impose overhead in low-dimensional contexts.

\begin{table}[H]
    \footnotesize
    \centering
    \caption{RGB 3D Validation Results (26K images, averaged across two queries)}
    \label{tab:rgb_validation}
    ...
    \begin{tabularx}{\columnwidth}{l >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X}
        \toprule
        \textbf{Structure} & \textbf{Insert (ms)} & \textbf{Search (ms)} & \textbf{Found} & \textbf{Recall} \\
        \midrule
        Linear Search & \textbf{1.00} & 2.32 & 8380 & 100\% \\
        Octree & 5.48 & \textbf{2.12} & 8380 & 100\% \\
        Quadtree & 6.92 & 2.25 & 8380 & 100\% \\
        Hash Dynamic & 4.14 & 2.15 & 8380 & 100\% \\
        LSH (L=10,k=4) & 53.91 & 4.62 & 8039 & 95.9\% \\
    ...
    \begin{minipage}{\columnwidth}
    \footnotesize
    \textit{Classical structures (Octree, Quadtree) dominate in RGB 3D space with simpler implementations achieving better performance and precision. LSH shows 2.2× search slowdown vs Octree. Threshold=40.0.}
    \end{minipage}
\end{table}
\vspace{-0.3cm}
