...
BenchmarkResult benchmarkStructure(std::unique_ptr<ImageDatabase> db, 
    ...
    result.insertTime = std::chrono::duration<double, std::micro>(endInsert - startInsert).count();
    
    // Teste de Busca
    auto startSearch = std::chrono::high_resolution_clock::now();
    auto results = db->findSimilar(query, threshold);
    auto endSearch = std::chrono::high_resolution_clock::now();
    result.searchTime = std::chrono::duration<double, std::micro>(endSearch - startSearch).count();
...
int main() {
    ...
    // Cabecalho da tabela
    printf("%-10s %-15s %-12s %-12s %-8s\n", "Dataset", "Estrutura", "Insert(μs)", "Search(μs)", "Found");
    std::cout << "-------------------------------------------------------------------------------\n";
    ...
    for (int scale : scales) {
        ...
        printf("%-10d | Insert: %-15s (%.1fμs) | Search: %-15s (%.1fμs)\n", 
